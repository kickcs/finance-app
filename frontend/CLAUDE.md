...
- **Mobile vs desktop**: the one and only breakpoint is 1024px, and it lives in `@/shared/lib/platform` — never branch platforms with `md:`/`lg:` classes or a local `useMediaQuery`. A split page keeps its mobile SFC where it is, puts the desktop one under `desktop/`, and shares logic through `model/use<Page>Page.ts`; the router wires both via `platformPage(mobile, desktop)` so each device only downloads its own chunk. Desktop pages build on `DesktopPage`/`DesktopColumns` and never carry `pb-28`. Overlays go through `UOverlay` (`desktop="panel" | "dialog"`), not hand-rolled vaul boilerplate
...
- **Runtime chunk cache**: everything outside the shell is served by the service worker's `route-chunks` CacheFirst rule, capped by `maxEntries` in `vite.config.ts`. Once the build emits more files than the cap, workbox evicts by LRU and rarely-visited pages stop opening offline — invisible online, invisible in the diff. Each new page adds its own chunk and CSS, so `scripts/check-runtime-cache-budget.mjs` (`bun run check:cache`) re-checks the headroom after every build
