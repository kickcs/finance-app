...
import { ReminderCard, ReminderCardSkeleton, type Reminder } from '@/entities/reminder'
  ...
  <div class="space-y-3">
    ...
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-2">
        ...
        <span
          v-if="todayCount > 0"
          class="px-1.5 py-0.5 text-[10px] font-semibold rounded-md bg-primary text-white"
        >
          {{ todayCount }}
        </span>
      </div>
    ...
    >
      <div class="w-10 h-10 mx-auto mb-2 rounded-lg bg-surface-light dark:bg-surface-dark flex items-center justify-center">
        <UIcon name="notifications" size="md" class="text-text-tertiary-light dark:text-text-tertiary-dark" />
      </div>
      <p class="text-sm text-text-secondary-light dark:text-text-secondary-dark mb-0.5">
        Нет подписок
      </p>
      <p class="text-xs text-text-tertiary-light dark:text-text-tertiary-dark mb-3">
