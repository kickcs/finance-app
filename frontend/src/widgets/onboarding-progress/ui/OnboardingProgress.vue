...
const hasFirstTransaction = computed(
  () => (transactions.value?.length ?? 0) > 0,
);
...
const progressPercentage = computed(() =>
  Math.round((stepsCompleted.value / 2) * 100),
);
...
<template>
  <div v-if="!isFullyOnboarded" class="mb-6">
    <UCard
      variant="default"
      class="relative overflow-hidden border-primary/20 bg-primary/5 dark:bg-primary/10"
    >
      <div class="p-4">
        <div class="flex justify-between items-start mb-3">
          ...
          <div class="flex items-center gap-2">
            ...
            >
            <button
              class="text-text-tertiary-light dark:text-text-tertiary-dark p-1 rounded-full hover:bg-surface-light dark:hover:bg-surface-dark transition-colors"
              @click="dismiss"
            >
        ...
        <div class="space-y-3">
          ...
          <div class="flex items-center gap-3 opacity-60">
            <div
              class="w-6 h-6 rounded-full bg-success/20 text-success flex items-center justify-center shrink-0"
            >
              <UIcon name="check" size="sm" />
            </div>
            <span
              class="text-sm text-text-primary-light dark:text-text-primary-dark line-through decoration-text-tertiary-light"
            >
              Создать первый счёт
            </span>
          </div>
          ...
          >
            <div
              ...
              :class="
                hasFirstTransaction
                  ...
                  : 'bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark text-text-tertiary-light'
              "
            >
            ...
            <div class="flex-1">
              <span
                ...
                :class="{
                  'line-through decoration-text-tertiary-light':
                    hasFirstTransaction,
                }"
              >
