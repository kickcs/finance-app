...
import { DebtCard, DebtCardSkeleton, type Debt, DEBT_DIRECTION_COLORS } from '@/entities/debt'
...
const debtsByPerson = computed<DebtByPerson[]>(() => {
  ...
  for (const debt of activeDebts.value) {
    ...
    if (debt.next_payment_date) {
      if (!grouped[key].nearestDueDate || new Date(debt.next_payment_date) < new Date(grouped[key].nearestDueDate!)) {
...
const overdueCount = computed(() => {
  const now = new Date()
  return activeDebts.value.filter(d => d.next_payment_date && new Date(d.next_payment_date) < now).length
})
...
<template>
  <div class="space-y-3">
    ...
    >
      <div class="flex items-center justify-between px-3 py-2 rounded-lg bg-surface-light dark:bg-surface-dark">
        <span class="text-xs text-text-tertiary-light dark:text-text-tertiary-dark">Вам</span>
        <span class="text-xs font-semibold text-warning">
          {{ formatCurrency(totalGivenDebts, currency) }}
        </span>
      </div>
      <div class="flex items-center justify-between px-3 py-2 rounded-lg bg-surface-light dark:bg-surface-dark">
        <span class="text-xs text-text-tertiary-light dark:text-text-tertiary-dark">Вы</span>
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3 min-w-0">
            <div
              ...
              :style="{ backgroundColor: `${DEBT_DIRECTION_COLORS[group.debtType]}12` }"
            >
              <UIcon
            ...
            <div class="min-w-0">
              ...
              <div class="flex items-center gap-1">
                <span class="text-caption-sm text-text-tertiary-light dark:text-text-tertiary-dark">
          ...
          <div class="text-right shrink-0">
            ...
            <p
              ...
              :class="isOverdue(group.nearestDueDate)
                ? 'text-danger font-medium'
                : 'text-text-tertiary-light dark:text-text-tertiary-dark'"
            >
              {{ isOverdue(group.nearestDueDate) ? 'Просрочено' : formatDate(group.nearestDueDate, { format: 'short' }) }}
            </p>
          </div>
        </div>
      </button>
    ...
    >
      <div class="w-10 h-10 mx-auto mb-2 rounded-lg bg-success-light flex items-center justify-center">
