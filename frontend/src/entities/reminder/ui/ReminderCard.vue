...
import { isReminderUpcoming, isReminderOverdue } from '../model/utils';
...
<template>
  <button
    ...
    :aria-label="`${reminder.name}, ${formatCurrency(reminder.amount, currency || DEFAULT_CURRENCY)}, ${nextDateFormatted}`"
    @click="$emit('click')"
  >
    <!-- Icon -->
    <div
    ...
    <div class="text-right shrink-0">
      <p class="text-sm font-semibold text-text-primary-light dark:text-text-primary-dark">
        {{ formatCurrency(reminder.amount, currency || DEFAULT_CURRENCY) }}
      </p>
      <p
        class="text-xs"
        :class="[
