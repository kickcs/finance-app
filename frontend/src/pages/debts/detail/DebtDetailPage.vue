        ...
        <UCard class="p-5">
          <div class="flex items-start gap-4">
            ...
            <div
              ...
              :style="{ backgroundColor: `${DEBT_DIRECTION_COLORS[debt.debt_type]}20` }"
            >
              <UIcon
          ...
          <div class="mt-6 pt-6 border-t border-border-light dark:border-border-dark">
            ...
            <div v-if="!debt.is_closed && debt.remaining_amount < debt.total_amount" class="space-y-2 mt-3">
              <UProgressBar
                :value="progress"
        ...
        <UCard variant="default" class="p-5 space-y-4">
          ...
          <div v-if="debt.next_payment_date" class="flex items-center justify-between">
            ...
            <span
              ...
              :class="new Date(debt.next_payment_date) < new Date() && !debt.is_closed
                ? 'text-danger'
                : 'text-text-primary-light dark:text-text-primary-dark'"
            >
              {{ formatDate(debt.next_payment_date, { format: 'short' }) }}
              <span v-if="new Date(debt.next_payment_date) < new Date() && !debt.is_closed" class="text-xs">
                (просрочено)
              </span>
            </span>
          </div>
