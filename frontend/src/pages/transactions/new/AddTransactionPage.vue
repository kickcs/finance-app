...
const { expenseCategories, incomeCategories, isLoading: categoriesLoading } = useCategories(userId)
const { profile, defaultAccountId } = useProfile(userId)
const userCurrency = computed(() => profile.value?.currency || 'UZS')
const isLoading = computed(() => accountsLoading.value || categoriesLoading.value)
...
async function handleSubmit() {
  ...
  if (transactionId) {
    ...
    if (splitData.value.enabled && splitData.value.participants.length > 0 && formData.value.accountId) {
      ...
      if (!success) {
        ...
        error.value = 'Не удалось создать долги для раздельного счёта. Операция отменена.'
        return
      }
    }
  ...
  <div class="min-h-screen bg-background-light dark:bg-background-dark pb-24">
    ...
    <header class="sticky top-0 z-30 pt-[var(--safe-area-inset-top)] bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-xl">
      <div class="flex items-center justify-between px-4 py-3">
        ...
        <h1 class="text-base font-semibold text-text-primary-light dark:text-text-primary-dark">
          Новая транзакция
        </h1>
        <div class="w-10" />
      </div>
    ...
    <main class="px-4 pt-4 pb-4">
      ...
      <div v-else-if="accounts.length === 0" class="text-center py-8">
        <UIcon name="account_balance_wallet" size="lg" class="text-text-tertiary-light dark:text-text-tertiary-dark mb-3" />
