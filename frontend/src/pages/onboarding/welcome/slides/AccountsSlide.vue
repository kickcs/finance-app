...
const accountTypes = [
  { icon: 'payments', color: 'success', label: 'Наличные', amount: '12 450 ₽' },
  { icon: 'credit_card', color: 'primary', label: 'Банковские карты', amount: '84 200 ₽' },
  { icon: 'account_balance', color: 'warning', label: 'Депозиты', amount: '500 000 ₽' },
  { icon: 'account_balance_wallet', color: 'debt-received', label: 'Кошельки', amount: '1 250 $' },
] as const

const features = [
  { icon: 'currency_exchange', text: 'Мультивалютная поддержка' },
  { icon: 'swap_horiz', text: 'Доходы, расходы и переводы' },
  { icon: 'group', text: 'Разделение расходов с друзьями' },
] as const
...
const iconBgClasses: Record<string, string> = {
  ...
  'debt-received': 'bg-debt-received/10',
}
...
const iconTextClasses: Record<string, string> = {
  ...
  'debt-received': 'text-debt-received',
}
</script>
...
<template>
  <div class="w-full h-full flex-shrink-0 flex flex-col px-6 relative z-10">
    ...
    <div class="w-full mb-8 relative z-10">
      <div class="grid grid-cols-2 gap-3">
        ...
        >
          ...
          <div>
            <p class="text-text-tertiary-dark text-[11px] font-semibold uppercase tracking-wider mb-1">{{ acc.label }}</p>
            <p class="text-xl font-bold text-text-primary-dark">{{ acc.amount }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-5 mt-auto mb-6 relative z-10">
      ...
      >
        <div class="flex-shrink-0 w-10 h-10 rounded-full bg-card-dark border border-white/5 flex items-center justify-center">
