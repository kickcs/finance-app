...
function openDeleteModal() {
  ...
  if (account.value && isDefaultAccount(account.value.id)) {
    ...
    accountError.value = 'Нельзя удалить счёт по умолчанию. Сначала назначьте другой счёт по умолчанию.'
  }
    ...
    <main class="px-5 pt-8 pb-6">
      ...
      <div v-else class="space-y-6">
        ...
        <UCard class="p-5">
          ...
          <!-- Actions -->
          <div class="mt-6 pt-6 border-t border-border-light dark:border-border-dark space-y-3">
            <!-- Set as default -->
            <UButton
              v-if="!isDefaultAccount(account.id)"
              variant="secondary"
