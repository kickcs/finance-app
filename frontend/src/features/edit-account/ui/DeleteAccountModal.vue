    ...
    <div v-if="account" class="space-y-4">
      ...
      <div v-if="error" class="p-4 rounded-xl bg-warning/10">
        <div class="flex items-start gap-3">
          <UIcon name="info" size="sm" class="text-warning mt-0.5 shrink-0" />
      ...
      <div v-else class="p-4 rounded-xl bg-danger/10">
        <div class="flex items-start gap-3">
          <UIcon name="warning" size="sm" class="text-danger mt-0.5 shrink-0" />
          <div class="space-y-1">
            ...
            <p class="text-sm text-danger">Это действие нельзя отменить.</p>
          </div>
        </div>
      </div>
    </div>
