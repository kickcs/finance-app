...
async function generateSplash(width: number, height: number, bgColor: string, outputPath: string) {
  const logoSize = Math.round(width * LOGO_SIZE_RATIO);

  const logoBuffer = await sharp(Buffer.from(LOGO_SVG)).resize(logoSize, logoSize).png().toBuffer();
