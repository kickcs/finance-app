...
**Дополнено при исполнении.** Замена ручной вёрстки на компоненты дизайн-системы
вскрыла три молчавших бага в самих компонентах, и без их починки подмена
выглядела бы регрессом. Значок долга в `DebtCard` красился инлайновым
`${debtColor}15`, а цвета направления приезжают как `var(--color-…)` — склейка с
альфой давала невалидный CSS, и подложки не было вовсе; теперь это токены
`bg-debt-given-light`/`bg-debt-received-light`. `UProgressBar` считал своим
цветом только хекс, поэтому полоса, покрашенная токеном, молча уезжала в
`primary`. `InitialAvatar` считал полупрозрачный фон конкатенацией хекса с
альфой — тот же изъян, вылеченный через `color-mix`. Полосе итоговой карточки
понадобился новый проп `trackColor`: две чаши одного итога — это дорожка и
заливка, а не два сегмента с зазором. Седьмым исправлением оказалось
подтверждение удаления: оно показывало имя и сумму скрытого долга мимо
`maskDebtName`.

Сегментная полоса `MutualDebtCard` осталась ручной осознанно: это не прогресс, а
четыре доли одного итога с приглушением зачтённых кусков — `UProgressBar` такого
не выражает, и подгонка его под этот случай была бы костылём в общем компоненте.

Хаптика разделена по смыслу, а не по слою. Намерение (`selection`) живёт там, где
происходит нажатие, — в `PaymentPresetRow`, `DebtCard`, `PersonDebtRow`,
`MutualDebtCard`; `openActions`/`openEdit` остались в `useDebtDetail`, потому что
их зовут оба хоста, и вынос в UI вернул бы ровно ту дубликацию, которую убрала
фаза 5. Отклик об исходе (`success`) остаётся в композаблах — в
`useDebtPaymentFlow`, `useEditDebt` и добавлен в удаление и отмену закрытия:
шторка закрывается до ответа сервера и своего перехода не увидит.

Тестовая заготовка долга заводилась в спеках по третьему разу, каждый раз со
своим набором колонок, — теперь одна: `src/test/fixtures/debt.ts`. Содержимое
`UModal` уезжает в портал, поэтому его текст читается из `document.body` общим
хелпером `bodyText()` (`src/test/test-utils.ts`), заодно нормализующим неразрывные
пробелы разрядов.
