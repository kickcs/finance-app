...
`InboxItem`: `id, type, amount, currency, merchant, cardMask, occurredAt|null, balanceAfter, status, transactionId, suggestedAccountId, createdAt`.
...
| `GetInbox` | `findPendingWithSuggestions` — pending (без `unparsed`), LEFT JOIN маппингов для `suggestedAccountId`, сортировка `occurredAt DESC` |
...
- **`model/types.ts`** — snake_case. `ImportedTransaction { id, type, amount|null, currency, merchant|null, card_mask, occurred_at: string|null, balance_after|null, status, transaction_id|null, suggested_account_id|null, created_at }`. Также `TelegramLinkStatus`, `TelegramCard`.
...
- **`confirm/ImportConfirmPage.vue`** — флоу подтверждения:
  - **Prefill** (watch на item, keyed `prefilledId` чтобы рефетч не затёр правки): income→income; balance_change с `amount<0`→expense, иначе→income; expense→expense; `date = occurred_at ?? now`; `description = merchant`; `accountId = suggested_account_id`.
...
```
presentation/controllers/  telegram-webhook.controller.ts, telegram-import.controller.ts
presentation/dto/          confirm-imported.dto.ts, set-card-mapping.dto.ts
application/commands/       ingest-bank-message, link-telegram-account, create-link-token,
                           ...
                           set-card-mapping, delete-card-mapping
