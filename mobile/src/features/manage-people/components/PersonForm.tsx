...
import { useCreatePerson, useUpdatePerson } from '@/entities/person/api/usePeople';
...
export function PersonForm({ initial, onClose }: Props) {
  const user = useAuthStore((s) => s.user);
  const createMutation = useCreatePerson(user?.id ?? null);
  const updateMutation = useUpdatePerson(user?.id ?? null);

  const [name, setName] = useState(initial?.name ?? '');
  const [color, setColor] = useState<string>(
    initial?.color ?? (ENTITY_COLORS[0] as string),
  );
      ...
      >
        ...
        <View className="flex-row items-center justify-between px-4 py-3">
          <Text className="text-lg font-semibold text-text-primary-light dark:text-text-primary-dark">
            {initial ? 'Изменить' : 'Новый человек'}
          </Text>
          <Pressable
            onPress={onClose}
